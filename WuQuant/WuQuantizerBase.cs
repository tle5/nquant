...
namespace WuQuant
{
    ...
    public abstract class WuQuantizerBase
    {
        ...
        private static PngEncoder pngEncoder = new PngEncoder { BitDepth = PngBitDepth.Bit8, ColorType = PngColorType.Palette};
        ...
        public byte[] QuantizeImage(byte[] data)
        {
        ...
        public Image<Rgba32> QuantizeImage(Image<Rgba32> image)
        {
            return QuantizeImage(image, 10, 70);
        }

        public Image<Rgba32> QuantizeImage(Image<Rgba32> image, int alphaThreshold, int alphaFader)
        {
            return QuantizeImage(image, alphaThreshold, alphaFader, null, 256);
        }

        public Image<Rgba32> QuantizeImage(Image<Rgba32> image, int alphaThreshold, int alphaFader, Histogram histogram, int maxColors)
        {
            var buffer = new ImageBuffer<Rgba32>(image);
        ...
        private static void BuildHistogram(Histogram histogram, ImageBuffer<Rgba32> sourceImage, int alphaThreshold, int alphaFader)
        {
        ...
        internal abstract Image<Rgba32> GetQuantizedImage(ImageBuffer<Rgba32> image, int colorCount, Pixel[] lookups, int alphaThreshold);
    }
}